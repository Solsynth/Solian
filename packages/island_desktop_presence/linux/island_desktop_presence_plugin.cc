...
G_DEFINE_TYPE(IslandDesktopPresencePlugin, island_desktop_presence_plugin, g_object_get_type())
...
static void island_desktop_presence_plugin_class_init(IslandDesktopPresencePluginClass* klass) {
  G_OBJECT_CLASS(klass)->dispose = island_desktop_presence_plugin_dispose;
}

static void island_desktop_presence_plugin_init(IslandDesktopPresencePlugin* self) {}

static void method_call_cb(FlMethodChannel* channel, FlMethodCall* method_call,
                           gpointer user_data) {
  IslandDesktopPresencePlugin* plugin = ISLAND_DESKTOP_PRESENCE_PLUGIN(user_data);
  island_desktop_presence_plugin_handle_method_call(plugin, method_call);
}

void island_desktop_presence_plugin_register_with_registrar(FlPluginRegistrar* registrar) {
  IslandDesktopPresencePlugin* plugin = ISLAND_DESKTOP_PRESENCE_PLUGIN(
