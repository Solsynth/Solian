  ...
  Future<SnCloudFile> uploadFile({
    ...
    return FileUploader.fileUploadLimiter.run(() async {
      ...
      tasks.updateTask(
        ...
        statusMessage: DriveUploadStage.label(DriveUploadStage.preparing),
      );
      ...
      if (localEncryptKey == null &&
        ...
        // Source, thumbnail and compression upload concurrently, so each
        // stage's progress is tracked separately and combined into a weighted
        // total. This keeps the reported progress monotonic even though the
        // stages finish out of order.
        var sourceProgress = 0.0;
        ...
        void reportStage(String stage, double stageProgress) {
          final current = tasks.getTask(taskId)?.metadata ?? {};
          switch (stage) {
            case DriveUploadStage.uploadingSource:
              sourceProgress = stageProgress;
            case DriveUploadStage.uploadingThumbnail:
              hasThumbnail = true;
              thumbnailProgress = stageProgress;
            case DriveUploadStage.uploadingCompression:
              hasCompression = true;
          ...
          final progress = switch (stage) {
            ...
            DriveUploadStage.uploadingThumbnail ||
            DriveUploadStage.uploadingCompression =>
              0.82 * sourceProgress +
                  (hasThumbnail ? 0.06 * thumbnailProgress : 0.0) +
                  (hasCompression ? 0.07 * compressionProgress : 0.0),
            _ => (tasks.getTask(taskId)?.progress ?? 0).clamp(0.0, 1.0),
          };
          tasks.updateTask(
            taskId,
            progress: progress,
        ...
        try {
          ...
          final s3Uploaded = await tryUploadViaS3Direct(
            ...
            onProgress: (progress, estimate) {
              ...
              if (progress != null) {
                reportStage(DriveUploadStage.uploadingSource, progress);
              }
            },
          );
      ...
      tasks.updateTask(
        taskId,
        statusMessage: DriveUploadStage.label(DriveUploadStage.hashing),
        metadata: {
          ...?tasks.getTask(taskId)?.metadata,
